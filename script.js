                    ...
                    <tbody>
                        ${quotes.map(quote => `
                            <tr>
                                ...
                                <td>${quote.details}</td>
                                <td><span class="quote-status ${quote.status}">${quote.status}</span></td>
                    ...
                    <tbody>
                        ${jobs.map(job => `
                            <tr>
                                ...
                                <td>${job.company}</td>
                                <td>${job.location}</td>
                                <td><span class="quote-status ${job.status || 'pending'}">${job.status || 'pending'}</span></td>
...
async function handleStatusUpdate(userId, newStatus) {
    try {
        ...
        // No need to refresh manually as the dropdown state is preserved.
    } catch (error) {
        // If the API call fails, refresh the list to show the original, correct state.
        renderAdminUsers();
    }
}

// --- NEW FEATURE: Handle Subscription Update ---
async function handleSubscriptionUpdate(userId, newStatus) {
    try {
        ...
        renderAdminUsers(); // Refresh on error
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    // This logic handles initialization on both admin.html and the login page (index.html)
