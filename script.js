...
function viewAIEstimate(estimationId) {
    ...
    trades.forEach(t => {
        ...
        if (t.lineItems && t.lineItems.length > 0) {
            lineRows = t.lineItems.map(li => `<tr><td>${li.description}${li.materialDetails ? `<div style="font-size:0.75rem;color:#8b5cf6;font-style:italic;">${li.materialDetails}</div>` : ''}${li.quantityBasis ? `<div style="font-size:0.72rem;color:#6366f1;font-style:italic;">📐 ${li.quantityBasis}</div>` : ''}</td><td>${Number(li.quantity || 0).toLocaleString()} ${li.unit || ''}</td><td>${curr}${Number(li.unitRate || li.unitTotal || 0).toLocaleString()}</td><td>${curr}${Number(li.lineTotal || 0).toLocaleString()}</td></tr>`).join('');
        }
