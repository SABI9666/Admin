...
// --- DASHBOARD (LEGACY - KEPT FOR FALLBACK) ---
...
async function renderEnhancedDashboard() {
    const contentArea = document.getElementById('admin-content-area');
    try {
        const [dashboardData, profileStats] = await Promise.all([
            apiCall('/admin/dashboard'),
            apiCall('/admin/profile-stats')
        ]);

        contentArea.innerHTML = `
            <div class="dashboard-overview">
                <div class="admin-stats-grid">
                    ${Object.entries(dashboardData.stats || dashboardData.data.stats).map(([key, value]) => `
                        <div class="admin-stat-card">
                            <div class="stat-icon">
                                <i class="fas fa-${getStatIcon(key)}"></i>
                            </div>
                    ...
                    <div class="admin-stat-card profile-stats">
                        <div class="stat-icon">
                            <i class="fas fa-user-check"></i>
                        </div>
                        <div class="stat-info">
                            <span class="stat-value">${profileStats.data.pending}</span>
                    ...
                    <div class="admin-stat-card profile-stats">
                        ...
                        <div class="stat-info">
                            <span class="stat-value">${profileStats.data.approved}</span>
                ...
                <div class="dashboard-actions">
                    ...
                    <div class="quick-actions-grid">
                        ${profileStats.data.pending > 0 ? `
                            <button class="btn btn-warning" onclick="document.querySelector('.admin-nav-link[data-section=\\'profile-reviews\\']').click()">
                                <i class="fas fa-user-check"></i>
                                Review ${profileStats.data.pending} Pending Profiles
                            </button>
                        ` : ''}
                        <button class="btn btn-primary" onclick="bulkApproveProfiles()">
                ...
                <div class="recent-activity">
                    ...
                    <div class="activity-summary">
                        <div class="activity-item">
                            ...
                            <span>${profileStats.data.pendingDesigners} designers awaiting review</span>
                        </div>
                        <div class="activity-item">
                            <i class="fas fa-clock text-warning"></i>
                            <span>${profileStats.data.pendingContractors} contractors awaiting review</span>
                        </div>
                        <div class="activity-item">
                            <i class="fas fa-check-circle text-success"></i>
                            <span>${profileStats.data.approved} total approved profiles</span>
                        </div>
                    </div>
                </div>
            </div>
...
async function renderEnhancedUsersTab() {
    const contentArea = document.getElementById('admin-content-area');
    try {
        // Assuming API returns { data: users } for this endpoint
        const { data: users } = await apiCall('/admin/users');
        ...
        contentArea.innerHTML = `
            <div class="admin-section-header">
                ...
                <div class="section-actions">
                    ...
                    <select id="user-status-filter" onchange="filterUsersByStatus(this.value)">
            ...
            <div class="admin-table-container">
                <table class="admin-table" id="users-table">
                    ...
                    <tbody>
                        ${users.map(user => `
                            <tr data-user-id="${user._id}" data-role="${user.type || user.role}" data-name="${user.name}" data-profile-status="${user.profileStatus || 'incomplete'}">
                                <td>
                                    <div class="user-info">
                                        <div class="user-avatar">
                                            ${user.avatar ?
                                        ...
                                        <div class="user-details">
                                            ...
                                            <small>${user.company || user.companyName || 'No company'}</small>
                                        </div>
                                    </div>
                                </td>
                                <td>${user.email}</td>
                                <td>
                                    <span class="role-badge ${user.type || user.role}">${user.type || user.role}</span>
                                </td>
                                <td>
                                    <span class="status-badge ${user.profileStatus || 'incomplete'}">
                                        ${getProfileStatusText(user.profileCompleted, user.profileStatus)}
                                    </span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="action-buttons">
                                        ...
                                        ${user.profileCompleted && user.profileStatus === 'pending' ? `
                                            <button class="btn btn-sm btn-success" onclick="quickApproveProfile('${user._id}')" title="Quick Approve">
                                                <i class="fas fa-check"></i>
                                            </button>
                                        ` : ''}
                                        <button class="btn btn-sm btn-warning" onclick="toggleUserStatus('${user._id}', ${user.isActive !== false ? 'false' : 'true'})" title="${user.isActive !== false ? 'Deactivate' : 'Activate'}">
        ...
        contentArea.innerHTML = `
            <div class="error-state">
                ...
                <button class="btn btn-primary" onclick="renderEnhancedUsersTab()">
                    <i class="fas fa-redo"></i> Retry
                </button>
            </div>
        `;
...
// Helper function for profile status text
function getProfileStatusText(profileCompleted, profileStatus) {
...
function filterUsersByStatus(status) {
    ...
    rows.forEach(row => {
        const profileStatus = row.dataset.profileStatus;
        const shouldShow = !status || profileStatus === status;
        row.style.display = shouldShow ? '' : 'none';
    });
}

// Quick approve profile from users tab
async function quickApproveProfile(userId) {
    if (!confirm('Approve this user\'s profile?')) return;
    
    try {
        await apiCall(`/admin/profile-reviews/${userId}/approve`, 'POST', {
            notes: 'Quick approval from users tab'
        });
        showNotification('Profile approved successfully!', 'success');
        renderEnhancedUsersTab();
    } catch (error) {
...
async function loadProfileReviews() {
    try {
        ...
        if (allReviews.length === 0) {
            ...
            try {
                // Assuming this API returns { data: [...] }
...
async function bulkApproveProfiles() {
    try {
        const { data: pendingUsers } = await apiCall('/admin/users?profileStatus=pending');
        if (pendingUsers.length === 0) {
            showNotification('No pending profiles to approve', 'info');
            return;
        }
        
        if (!confirm(`Approve ${pendingUsers.length} pending profiles?`)) return;
        
        for (const user of pendingUsers) {
            await apiCall(`/admin/profile-reviews/${user._id}/approve`, 'POST', {
                notes: 'Bulk approval'
            });
        }
        
        showNotification(`${pendingUsers.length} profiles approved!`, 'success');
...
async function exportAllData() {
    try {
        ...
        const exportData = {
            ...
            profileReviews: await apiCall('/admin/profile-reviews'),
            jobs: await apiCall('/admin/jobs'),
            quotes: await apiCall('/admin/quotes'),
            messages: await apiCall('/admin/messages'),
        ...
        a.href = url;
        a.download = `steelconnect-admin-export-${new Date().toISOString().split('T')[0]}.json`;
...
async function systemHealthCheck() {
    try {
        ...
        const checks = [
            { name: 'Dashboard', endpoint: '/admin/dashboard' },
            { name: 'Users', endpoint: '/admin/users' },
            { name: 'Profile Reviews', endpoint: '/admin/profile-reviews' },
            { name: 'Jobs', endpoint: '/admin/jobs' },
            { name: 'Messages', endpoint: '/admin/messages' }
        ];

        const results = [];
        for (const check of checks) {
            try {
                await apiCall(check.endpoint);
        ...
        const healthModal = `
            <div class="modal-header premium-modal-header">
                <h3><i class="fas fa-heartbeat"></i> System Health Check Results</h3>
            </div>
            <div class="health-results">
                ${results.map(result => `
                    <div class="health-item ${result.status}">
                        <i class="fas fa-${result.status === 'healthy' ? 'check-circle' : 'exclamation-circle'}"></i>
                        <span class="health-name">${result.name}</span>
                        <span class="health-status">${result.status}</span>
                        ${result.error ? `<small class="health-error">${result.error}</small>` : ''}
                    </div>
                `).join('')}
            </div>
            <div class="modal-actions">
        ...
        `;
        showGenericModal(healthModal, 'max-width: 600px;');

        const healthyCount = results.filter(r => r.status === 'healthy').length;
        showNotification(`Health check complete: ${healthyCount}/${results.length} services healthy`, 
                        ...
                        ${messages.map(message => {
                            ...
                            return `
                                <tr class="${rowClass}" data-message-id="${messageId}"
                                    data-status="${isBlocked ? 'blocked' : (message.status || (message.isRead ? 'read' : 'unread'))}"
...
// --- DYNAMIC CSS INJECTION FOR NEW FEATURES ---
function injectNewStyles() {
    ...
    .profile-review-card.review-status-pending {
        border-left-color: #f59e0b;
        background: linear-gradient(to right, #fef3c7, #ffffff);
    }

    .profile-review-card.review-status-approved {
        border-left-color: #10b981;
        background: linear-gradient(to right, #d1fae5, #ffffff);
    }

    .profile-review-card.review-status-rejected {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.getElementById('admin-panel-container')) {
        initializeAdminPage();
        injectNewStyles(); // Inject styles for new features
    }
    ...
    console.log('✅ Enhanced admin profile management system loaded');
});
