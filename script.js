...
function logout() {
    ...
    setTimeout(() => window.location.href = 'login.html', 1000); // Corrected: Redirect to login page
}
...
async function handleAdminLogin(event) {
    ...
    try {
        ...
        setTimeout(() => { window.location.href = 'index.html'; }, 1000); // Redirect to admin panel
    } catch (error) {
...
function initializeAdminPage() {
    ...
    if (token && userJson) {
        try {
            ...
            // More robust security check (case-insensitive)
...
function renderAdminSection(section) {
    ...
    switch (section) {
        ...
        case 'estimations': renderEstimations(); break; // Added estimations
...
// Placeholder for your new estimations feature
function renderEstimations() {
    const contentArea = document.getElementById('admin-content-area');
    contentArea.innerHTML = `<div class="empty-state">Estimations feature coming soon!</div>`;
}

// Other render functions (renderUsers, renderQuotes, etc.) would go here...
