...
function viewAIEstimate(estimationId) {
    ...
    // Admin helper: build a collapsible table for generic material items with prices
    function adminMatTable(items, headerBg, cols) {
        return `<div style="overflow-x:auto;"><table style="width:100%;font-size:0.78rem;border-collapse:collapse;">
            <thead><tr style="background:${headerBg};">${cols.map(c => `<th style="padding:4px 6px;${c.align ? 'text-align:'+c.align : 'text-align:left'}">${c.label}</th>`).join('')}<th style="text-align:right;">Rate</th><th style="text-align:right;">Cost</th></tr></thead>
            <tbody>${items.map(item => `<tr style="border-bottom:1px solid #f1f5f9;">${cols.map(c => `<td style="padding:3px 6px;${c.style || ''}">${c.render ? c.render(item) : (item[c.key] || '-')}</td>`).join('')}
                <td style="text-align:right;">${curr}${Number(item.unitRate || 0).toLocaleString()}</td>
                <td style="text-align:right;font-weight:600;">${curr}${Number(item.totalCost || 0).toLocaleString()}</td></tr>`).join('')}</tbody>
            <tfoot><tr style="background:${headerBg};font-weight:700;"><td colspan="${cols.length}">Subtotal</td><td></td><td style="text-align:right;">${curr}${Number(items.reduce((s,i) => s + (Number(i.totalCost) || 0), 0)).toLocaleString()}</td></tr></tfoot>
        </table></div>`;
    }
    ...
    if (hasAnyMatSched) {
        ...
        if (steelMbrs.length > 0) {
            ...
            <div style="overflow-x:auto;"><table style="width:100%;font-size:0.78rem;border-collapse:collapse;">
                <thead><tr style="background:#eff6ff;"><th style="padding:4px 6px;">Mark</th><th>Section</th><th>Count</th><th>Length</th><th style="text-align:right;">Tons</th><th style="text-align:right;">Rate</th><th style="text-align:right;">Cost</th></tr></thead>
                <tbody>${steelMbrs.map(m => `<tr style="border-bottom:1px solid #f1f5f9;">
                    ...
                    <td style="text-align:right;">${m.unitRate?curr+Number(m.unitRate).toLocaleString():'-'}</td>
        ...
        if (concItems.length > 0) {
            ...
            <div style="overflow-x:auto;"><table style="width:100%;font-size:0.78rem;border-collapse:collapse;">
                <thead><tr style="background:#f0fdf4;"><th style="padding:4px 6px;">Element</th><th>Dims</th><th>Count</th><th style="text-align:right;">CY</th><th style="text-align:right;">Rate</th><th style="text-align:right;">Cost</th></tr></thead>
                <tbody>${concItems.map(c => `<tr style="border-bottom:1px solid #f1f5f9;">
                    ...
                    <td style="text-align:right;">${c.unitRate?curr+Number(c.unitRate).toLocaleString():'-'}</td>
